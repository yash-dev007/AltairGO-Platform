...
export default function SummaryTab({
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: '1.5rem' }}>
      ...
      {tripSummary ? (
        <>
          ...
          <div style={{ background: 'white', borderRadius: '16px', padding: '1.5rem', boxShadow: '0 2px 8px rgba(0,0,0,0.04)' }}>
            <h3 style={{ fontWeight: 700, color: '#141413', marginBottom: '1.25rem', fontSize: '1rem' }}>Trip Summary</h3>
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(150px, 1fr))', gap: '1rem', marginBottom: '1.25rem' }}>
              {[
                ...
                <div key={i} style={{ background: '#f5f4ed', borderRadius: '12px', padding: '1rem', textAlign: 'center' }}>
                  <div style={{ fontSize: '0.75rem', color: '#87867f', fontWeight: 600, textTransform: 'uppercase', marginBottom: '0.4rem' }}>{m.label}</div>
                  <div style={{ fontSize: '1.4rem', fontWeight: 800, color: '#141413' }}>{m.value ?? '—'}</div>
                </div>
              ))}
            </div>
            ...
            {tripSummary.highlights?.length > 0 && (
              <div>
                <div style={{ fontWeight: 600, color: '#5e5d59', fontSize: '0.85rem', marginBottom: '0.5rem' }}>Highlights</div>
            ...
            {tripSummary.top_activity_types?.length > 0 && (
              <div style={{ marginTop: '1rem' }}>
                <div style={{ fontWeight: 600, color: '#5e5d59', fontSize: '0.85rem', marginBottom: '0.5rem' }}>What you loved doing</div>
                <div style={{ display: 'flex', flexWrap: 'wrap', gap: '0.5rem' }}>
          ...
          {tripSummary.smart_insights?.length > 0 && (
            <div style={{ background: 'white', borderRadius: '16px', padding: '1.5rem', boxShadow: '0 2px 8px rgba(0,0,0,0.04)' }}>
              <h3 style={{ fontWeight: 700, color: '#141413', marginBottom: '1rem', fontSize: '1rem' }}>Insights</h3>
        ...
        <div style={{ background: 'white', borderRadius: '16px', padding: '2rem', textAlign: 'center', color: '#87867f', boxShadow: '0 2px 8px rgba(0,0,0,0.04)' }}>
      ...
      {/* Review form */}
      <div style={{ background: 'white', borderRadius: '16px', padding: '1.5rem', boxShadow: '0 2px 8px rgba(0,0,0,0.04)' }}>
        <h3 style={{ fontWeight: 700, color: '#141413', marginBottom: '0.25rem', fontSize: '1rem' }}>
          {existingReview ? 'Your Review' : 'Rate This Trip'}
        </h3>
        <p style={{ fontSize: '0.85rem', color: '#87867f', marginBottom: '1.25rem' }}>
        ...
        <form onSubmit={onSubmitReview}>
            ...
            <div style={{ display: 'flex', gap: '0.5rem', marginBottom: '0.5rem' }}>
              {[1, 2, 3, 4, 5].map(n => (
                <button
                  ...
                  style={{ background: 'none', border: 'none', cursor: 'pointer', padding: '4px', fontSize: '1.8rem', lineHeight: 1, color: n <= reviewForm.rating ? '#0BA060' : '#f0eee6', transition: 'color 0.15s' }}
                >
                  ★
                </button>
              ))}
          ...
          {/* Tags */}
          <div style={{ marginBottom: '1.25rem' }}>
            <div style={{ fontWeight: 600, fontSize: '0.85rem', color: '#5e5d59', marginBottom: '0.5rem' }}>Tags (up to 5)</div>
            <div style={{ display: 'flex', flexWrap: 'wrap', gap: '0.4rem' }}>
              {REVIEW_TAGS.map(tag => (
                <button
                  ...
                  style={{ padding: '4px 12px', borderRadius: '999px', fontSize: '0.8rem', fontWeight: 600, cursor: 'pointer', fontFamily: 'inherit', background: reviewForm.tags.includes(tag) ? '#141413' : '#faf9f5', color: reviewForm.tags.includes(tag) ? 'white' : '#5e5d59', border: 'none' }}
                >
                  {tag}
                </button>
              ))}
          ...
          <div style={{ marginBottom: '1.25rem' }}>
            <textarea
              ...
              maxLength={2000}
              rows={4}
              style={{ width: '100%', padding: '0.75rem 1rem', border: '1.5px solid #f0eee6', borderRadius: '12px', fontFamily: 'inherit', fontSize: '0.9rem', outline: 'none', resize: 'vertical', lineHeight: 1.6, color: '#141413', boxSizing: 'border-box' }}
            />
          </div>
          ...
          <button
            ...
            style={{ padding: '0.75rem 2rem', background: submittingReview || !reviewForm.rating ? '#87867f' : '#141413', color: 'white', border: 'none', borderRadius: '10px', fontFamily: 'inherit', fontWeight: 600, cursor: submittingReview || !reviewForm.rating ? 'not-allowed' : 'pointer', fontSize: '0.9rem' }}
          >
