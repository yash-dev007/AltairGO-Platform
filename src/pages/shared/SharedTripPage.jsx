...
const SharedTripPage = () => {
  ...
  if (error || !trip) return (
    <div style={{ textAlign: 'center', padding: '8rem 2rem' }}>
      ...
      <p style={{ color: '#5e5d59', marginBottom: '2rem' }}>This shared trip link may have expired.</p>
      <Link to="/" style={{ color: '#141413', fontWeight: 600, textDecoration: 'none', background: '#faf9f5', padding: '0.75rem 1.5rem', borderRadius: '50px' }}>Go Home</Link>
    </div>
  );
  ...
  return (
    <div style={{ minHeight: '100vh', background: '#f5f4ed' }}>
      {/* Shared Banner */}
      <div style={{ background: '#141413', padding: '1rem 2rem', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
        <div style={{ color: 'white', fontWeight: 800, fontSize: '1.1rem' }}>Altair<span style={{ color: '#5ac576' }}>GO</span></div>
        <div style={{ display: 'flex', alignItems: 'center', gap: '10px', color: 'rgba(255,255,255,0.7)', fontSize: '0.85rem' }}>
          <Sparkles size={14} color="#5ac576" /> Shared itinerary — read only
        </div>
        <Link to="/planner" style={{ background: '#5ac576', color: '#141413', padding: '0.5rem 1.25rem', borderRadius: '50px', fontWeight: 700, fontSize: '0.85rem', textDecoration: 'none' }}>
          Plan My Trip
        </Link>
      </div>
      ...
      <div style={{ maxWidth: '900px', margin: '0 auto', padding: '3rem 1.5rem' }}>
        ...
        <div style={{ background: 'white', borderRadius: '20px', padding: '2rem', boxShadow: '0 4px 20px rgba(0,0,0,0.06)', marginBottom: '2rem' }}>
          <h1 style={{ fontSize: 'clamp(1.5rem, 4vw, 2.25rem)', fontWeight: 800, color: '#141413', marginBottom: '1rem' }}>{title}</h1>
          ...
          {insights.length > 0 && (
            <div style={{ marginTop: '1.5rem', paddingTop: '1.5rem', borderTop: '1px solid #faf9f5' }}>
              <h3 style={{ fontWeight: 700, color: '#141413', marginBottom: '0.75rem', fontSize: '0.9rem' }}>Smart Insights</h3>
              {insights.map((insight, i) => (
                <div key={i} style={{ display: 'flex', gap: '8px', fontSize: '0.88rem', color: '#5e5d59', marginBottom: '0.4rem' }}>
                  <span style={{ color: '#5ac576' }}>•</span>
              ...
              <div key={dayNum} style={{ background: 'white', borderRadius: '16px', overflow: 'hidden', boxShadow: '0 2px 8px rgba(0,0,0,0.04)' }}>
                ...
                >
                  <div style={{ display: 'flex', gap: '1rem', alignItems: 'center' }}>
                    <div style={{ width: '36px', height: '36px', borderRadius: '50%', background: '#faf9f5', display: 'flex', alignItems: 'center', justifyContent: 'center', fontWeight: 800, color: '#141413', fontSize: '0.85rem', flexShrink: 0 }}>
                      {dayNum}
                    </div>
                    <div style={{ fontWeight: 700, color: '#141413' }}>
                      Day {dayNum}{day.theme ? ` — ${day.theme}` : ''}
                    </div>
                  </div>
                  {isExpanded ? <ChevronUp size={18} color="#87867f" /> : <ChevronDown size={18} color="#87867f" />}
                </div>

                {isExpanded && activities.length > 0 && (
                  <div style={{ borderTop: '1px solid #faf9f5' }}>
                    {activities.map((act, ai) => (
                      <div key={ai} style={{ padding: '0.9rem 1.5rem', borderBottom: ai < activities.length - 1 ? '1px solid #f5f4ed' : 'none', display: 'flex', gap: '1rem', alignItems: 'flex-start' }}>
                        <span style={{ fontSize: '0.78rem', fontWeight: 700, color: '#87867f', minWidth: '60px', paddingTop: '2px' }}>{act.time || ''}</span>
                        <div>
                          <div style={{ fontWeight: 700, color: '#141413', fontSize: '0.9rem' }}>{act.name}</div>
                          {act.description && <p style={{ fontSize: '0.82rem', color: '#5e5d59', marginTop: '3px', lineHeight: 1.5 }}>{act.description}</p>}
                          {act.cost > 0 && <div style={{ fontSize: '0.8rem', fontWeight: 600, color: '#5e5d59', marginTop: '4px' }}>₹{Number(act.cost).toLocaleString('en-IN')}</div>}
                        </div>
                      </div>
                    ))}
                  </div>
        ...
        {/* CTA */}
        <div style={{ marginTop: '3rem', textAlign: 'center', background: '#141413', borderRadius: '20px', padding: '2.5rem' }}>
          ...
          <Link to="/planner" style={{ display: 'inline-flex', alignItems: 'center', gap: '8px', background: '#5ac576', color: '#141413', padding: '1rem 2.5rem', borderRadius: '50px', fontWeight: 700, textDecoration: 'none' }}>
